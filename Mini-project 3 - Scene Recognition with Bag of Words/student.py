...
def get_tiny_images(image_paths):
    ...
    m = 16        # Resize to
    tiny_imgs = np.zeros((len(image_paths), m*m))
    
    for i, img_path in enumerate(image_paths):
        # Read image as gray
        img = rgb2grey(imread(img_path)) 
        # Resize it, then flatten the 16*16 array
        img = resize(img, (m, m)).reshape((-1,))
        # Normalize
        tiny_imgs[i] = img/np.linalg.norm(img)
...
def build_vocabulary(image_paths, vocab_size):
    ...
    cells_per_block = (z, z)   
    pixels_per_cell = (4, 4)  
    features = []     # For storing all feature vectors
    
    for img_path in image_paths:
        img = rgb2grey(imread(img_path))
        feature_vectors = hog(img, pixels_per_cell=pixels_per_cell,
    ...
    # Clustering to find a smaller representative points
    clf = MiniBatchKMeans(n_clusters=vocab_size, max_iter=100, tol=1e-3)
    clf.fit(features)
    vocab = clf.cluster_centers_
    # Make sure it's the right dimensions
    assert vocab.shape == (vocab_size, 9*z*z), "Incorrect output dimensions"
...
def get_bags_of_words(image_paths):
    ...
    for img_path in image_paths:
        ...
        distances = cdist(vocab, feature_vectors)      # Euclidean
        indicies = np.argsort(distances, axis=0)[0,:]  # Sorted for each word in ascending order
        
        # Creating image's histogram
        #hist = np.zeros(BINS_NUM)
        hist, _ = np.histogram(indicies, bins=BINS_NUM)
        bags_of_words.append(hist/np.linalg.norm(hist))
    bags_of_words = np.array(bags_of_words)
    assert bags_of_words.shape == (len(image_paths), BINS_NUM), "Incorrect output shape"
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    # TODO: Implement this function!
    clf = LinearSVC(tol=1e-5, C=2.0, random_state=0, max_iter=100)
